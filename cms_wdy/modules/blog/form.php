<?php
    ...
    */
    ...
    if(isset($_POST['save']))
    {
        $fields = array('title','page_title','category','author','publish_date','teaser','content','meta_description','status');
    	...
    	if(isset($_FILES['image']) && !empty($_FILES['image']['tmp_name'])) {
    		...
    		$image->toFile(UPLOADS_DIR . 'blog/' .$table_id . '-main-image.' . $imgData['extension']);
...
<form class="validate-form" method="post" enctype="multipart/form-data">
    ...
    <input name="slug" id="slug" size="50" type="hidden" value="<?php echo isset($data['slug']) ? $data['slug'] : ''; ?>" />
    <input name="url" id="url" size="50" type="hidden" value="<?php echo isset($data['url']) ? $data['url'] : ''; ?>" />
    <div class="card mb-4">
        <div class="card-header">
            <h1><?php echo ($table_id == 0) ? 'Add' : 'Edit'; ?> Blog / Article</h1>
        </div>
        <div class="card-body">
            <div class="form-group">
                <label for="image">Image: <span style="color: red;">(1000px by 1000px)</span></label>
                <input size="40" type="file" id="image" name="image" value="" class="form-control"  />
            </div>
            <div class="form-group">
                <?php    
                if($table_id > 0) {  
            	    $path = get_image('blog/' . $data['id'] . '-main-image');

            	    if (strlen($path) > 0):
                        ?>
               
            	        <?php show_image('blog/' . $data['id'] . '-main-image'); ?>
            ...
            <div class="form-group">
                ...
                <select name="category" class="form-control" id="category">
                    <?php $categories = table_fetch_rows('categories_posts','status = 1');
                    foreach($categories as $category){?>
                        <option value="<?= $category['id'];?>" <?php echo (isset($data['category']) && $data['category'] == $category['id']) ? 'selected="selected"' : ''; ?> ><?= $category['name'];?></option>
                    <?php } ?>
                </select>
            </div>
            <div class="form-group">
...
$(function() {
    ...
    function getCatSlug(table,category_id) {
        $.ajax({
            type: "POST",
            url: './ajax/get-category-slug.php',
            data: { 
    ...
    getCatSlug('categories_posts',category_id);
